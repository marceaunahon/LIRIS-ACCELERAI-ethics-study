...
class ChoiceSituationGUI(GeneralGUI):
    ...
    def __init__(self, situation_list: List[ChoiceSituation], title : str = "GUI", 
        ...
        self.option1_frame = OptionFrame(parent = self, text=self.situation.option1, _relx=0.28, _rely=0.5, _relwidth=0.4, 
                                         _relheight=0.55, option_number=0)
        self.option2_frame = OptionFrame(parent = self, text=self.situation.option2, _relx=0.72, _rely=0.5, _relwidth=0.4, 
                                         _relheight=0.55, option_number=1)
    ...
    def change_color(self):
        if self.choice.get() == 0 :
            self.option1_frame.configure(border_color = "#33AE81")
            self.option1_frame.textbox.configure(border_color = "#33AE81")
            self.option2_frame.configure(border_color = "#C0C0C0")
            self.option2_frame.textbox.configure(border_color = "#C0C0C0")
        if self.choice.get() == 1 :
            self.option2_frame.configure(border_color = "#33AE81")
            self.option2_frame.textbox.configure(border_color = "#33AE81")
            self.option1_frame.configure(border_color = "#C0C0C0")
...
class UpperFrame(ctk.CTkFrame):
    ...
    def __init__(self, parent : ChoiceSituationGUI):
        ...
        self.place(x=0.05, y=0, relwidth = 1, relheight = 0.25)
        self.textbox = TextBox(self, GUI=parent, text=parent.situation.choice_statement,
                                _relx=0.5, _rely=0.5, _relwidth=0.84,_relheight=0.65, _color="#33AE81") #on place le texte

    def reset_upper_frame(self, GUI : ChoiceSituationGUI):
        #self.textbox.reset(GUI) #on insère le nouveau texte
        pass

class OptionFrame(ctk.CTkFrame):
    def __init__(self, parent : ChoiceSituationGUI, text, _relx, _rely, _relwidth, _relheight, option_number):
        ...
        number_list = ["Première", "Deuxième"]
        self.text = text
        self.option_number = option_number
        self.button = ctk.CTkRadioButton(master=self, text = "Choisir cette option", variable=parent.choice,
                                        value=self.option_number, command=parent.change_color)
        if parent.disp_values : self.title = f"{number_list[self.option_number]} option : {parent.situation.value1}"
        if not parent.disp_values : self.title = f"{number_list[self.option_number]} option"
        self.place(relx=_relx, rely=_rely, relwidth = _relwidth, relheight = _relheight, anchor="center")
        self.textbox = TextBox(parent = self, GUI = parent, text=self.text,
                               _relx=0.5, _rely=0.79, _relwidth = 0.95, _relheight=0.23)
        self.image_canvas = ImageCanvas(parent = self, GUI = parent,
                               _relx=0.5, _rely=0.38, _relwidth = 0.58, _relheight=0.58,
                               im_number=option_number)
        self.label = ctk.CTkLabel(master=self, text=self.title, font = ctk.CTkFont(size=20, weight="bold"))
...
class EvalFrame(ctk.CTkFrame):
    ...
    def __init__(self, GUI : ChoiceSituationGUI):
        ...
        self.place(relx=0.5, rely=0.89, relwidth = 0.4, relheight = 0.2, anchor="center")
...
class TextBox(ctk.CTkTextbox):
    ...
    def __init__(self, parent, GUI : ChoiceSituationGUI, text, _relx, _rely, _relwidth, _relheight, _color="#C0C0C0"):
        #On construit la textbox
        font = ctk.CTkFont(family='Calibri', size = 20)
        super().__init__(master=parent, wrap="word", font=font, border_color=_color, border_width =0)
        self.text = text
        self.place(relx=_relx, rely=_rely, relwidth = _relwidth, relheight = _relheight, anchor="center")
        #self.reset() ##on affiche les valeurs si disp_values = True
    ...
    def reset(self, GUI : ChoiceSituationGUI, text):
        self.text = text
...
class ImageCanvas(ctk.CTkCanvas):
    ...
    def __init__(self, parent, GUI : ChoiceSituationGUI, _relx, _rely, _relwidth, _relheight, im_number):
